...
Rather than fitting our data structures into Numpy's types, we built new types out of basic arrays.

\begin{columns}
\column{1.07\linewidth}
\small
\begin{minted}{python}
>>> a = uproot.open("cms-nanoaod.root")["Events"].array("Muon_pt")
>>> a
\end{minted}
...
\vspace{-0.6 cm}
\begin{minted}{python}
>>> type(a)              # not really a Numpy array
\end{minted}
...
\vspace{-0.6 cm}
\begin{minted}{python}
>>> a.counts, a.content  # two contiguous arrays, not millions of them
\end{minted}
...
\vspace{-0.6 cm}
\begin{minted}{python}
>>> a[a.counts > 0]      # filter by elementwise comparison, like Numpy
\end{minted}
...
\vspace{-0.6 cm}
\begin{minted}{python}
>>> a[a.counts > 0, 0]   # jagged inner data are a slicable dimension
\end{minted}
